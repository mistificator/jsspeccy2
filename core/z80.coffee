...
window.JSSpeccy.buildZ80 = (opts) ->
	...
	getParamBoilerplate = (param, hasIXOffsetAlready = false) ->
		if param.match(/^[AFBCDEHL]|I[XY][HL]$/)
			...
			{
				'getter': "var val = READMEM(regPairs[#{rpPC}]++);"
				'v': 'val'
				'trunc': '& 0xff'
				'setter': ''
			}
	...
	CALL_C_NN = (flag, sense) ->
		...
		"""
			if (#{condition}) {
				var l = READMEM(regPairs[#{rpPC}]++);
				var h = READMEM(regPairs[#{rpPC}]);
				CONTEND_READ_NO_MREQ(regPairs[#{rpPC}]++, 1);
				WRITEMEM(--regPairs[#{rpSP}], regPairs[#{rpPC}] >> 8);
				WRITEMEM(--regPairs[#{rpSP}], regPairs[#{rpPC}] & 0xff);
				regPairs[#{rpPC}] = (h<<8) | l;
			} else {
	...
	EX_iSPi_RR = (rp) ->
		"""
			...
			regPairs[#{rp}] = (h<<8) | l;
			CONTEND_WRITE_NO_MREQ(regPairs[#{rpSP}], 2);
		"""
	...
	INIR_INDR = (modifier) ->
		"""
			#{INI_IND(modifier)}
			if (regs[#{rB}]) {
				CONTEND_WRITE_NO_MREQ(originalHL, 5);
				regPairs[#{rpPC}] -= 2;
			}
		"""
	...
	JR_C_N = (flag, sense) ->
		...
		"""
			if (#{condition}) {
				...
				CONTEND_READ(regPairs[#{rpPC}]++, 3); /* ++ to skip past offset byte */
			}
		"""
	...
	OUT_iCi_R = (r) ->
      ...
      var port = regPairs[#{rpBC}];
			CONTEND_PORT_EARLY(port);
			ioBus.write(port, regs[#{r}], tstates);
			CONTEND_PORT_LATE(port);
		"""

	OUT_iNi_A = () ->
		"""
			var port = (regs[#{rA}] << 8) | READMEM(regPairs[#{rpPC}]++);
			CONTEND_PORT_EARLY(port);
	...
	OUTI_OUTD = (modifier) ->
      ...
      var port = regPairs[#{rpBC}];
			CONTEND_PORT_EARLY(port);
			ioBus.write(port, outitemp, tstates);
			CONTEND_PORT_LATE(port);
	...
	PUSH_RR = (rp) ->
		"""
			CONTEND_READ_NO_MREQ(regPairs[#{rpIR}], 1);
			WRITEMEM(--regPairs[#{rpSP}], regPairs[#{rp}] >> 8);
			WRITEMEM(--regPairs[#{rpSP}], regPairs[#{rp}] & 0xff);
		"""
  ...
  ###
	opcodeSwitch = (runStringTable, fallbackTable = {}, traps = []) ->
		clauses = []
		for i, runString of runStringTable
			i = parseInt(i, 16)    
...
# usual calls      
	...
	defineZ80JS = """
      ...
      var opcodes = [];
      var opcodes_raw = [];
      var opcode_stat_frames = 250, opcode_stat_count = 0;
        ...
        if (opts.collectOpcodesStats) {
          if (opcode_stat_count >= opcode_stat_frames) {        
            ...
            for (var opcode of opcodes_raw) {
              map.set(opcode, (map.get(opcode) || 0) + 1);
            }
            map_sort = new Map([...map.entries()].sort((a, b) => b[1] - a[1]));
            console.log(map_sort);
            opcodes = Array.from(map.keys());
            opcodes_raw = [];
            opcode_stat_count = 0;
          } else opcode_stat_count++;
        }
			};
	...
	defineZ80JS = defineZ80JS.replace(/READMEM\((.*?)\)/g, '(CONTEND_READ(tmp_addr = $1, 3), memory.read(tmp_addr))');
	defineZ80JS = defineZ80JS.replace(/WRITEMEM\((.*?),(.*?)\)/g, """
		CONTEND_WRITE(tmp_addr = $1, 3);
		while (display.nextEventTime != null && display.nextEventTime < tstates) display.doEvent();
		memory.write(tmp_addr,$2);
	""");
	if opts.applyContention
		...
		defineZ80JS = defineZ80JS.replace(/CONTEND_READ\((.*?),(.*?)\)/g, '$1, tstates += ($2)');
		defineZ80JS = defineZ80JS.replace(/CONTEND_WRITE\((.*?),(.*?)\)/g, '$1, tstates += ($2)');
		defineZ80JS = defineZ80JS.replace(/CONTEND_READ_NO_MREQ\((.*?),(.*?)\)/g, '$1, tstates += ($2)');
		defineZ80JS = defineZ80JS.replace(/CONTEND_WRITE_NO_MREQ\((.*?),(.*?)\)/g, '$1, tstates += ($2)');
