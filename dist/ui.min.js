...
JSSpeccy.UI=function(a){function b(){h.isRunning?B.removeClass("start").addClass("stop"):B.removeClass("stop").addClass("start")}function d(a){z.toggleClass("enabled",a)}function e(){C.val(h.getModel().id)}function c(a){a?(v.attr("checked",!0),$(g).addClass("hard-edged-pixels")):(v.removeAttr("checked"),$(g).removeClass("hard-edged-pixels"))}function k(a){$(".panel",g).hide();$(".panel",g).filter(a).show();h.deactivateKeyboard()}function f(){$(".panel",g).hide();h.activateKeyboard()}function l(a,
...
$("button.joystick_keys",m).click(function(){h.stop();k(".joystick_keys")});$("button.pokes",m).click(function(){h.stop();$("#error_text",w).text("");k(".pokes")});$("button.run",m).click(function(){f();h.stop();h.setLoadUrlOnStart($("#links").children(":selected").attr("href"));h.start()});$("#links").change(function(){f();h.isRunning||t||h.setLoadUrlOnStart($("#links").children(":selected").attr("href"));A.val($("#links").children(":selected").attr("href"))});$("#jsspeccy-fps",g).html('<div>Video FPS: <span class="fps">0.0</span></div><div>CPU FPS: <span class="cfps">0.0</span></div>');
var x=function(){var a=navigator.userAgent.toLowerCase(),b={},c=a.match(/(android)\s+([\d.]+)/),d=a.match(/(iphone\sos)\s([\d_]+)/),e=a.match(/(ipad).*os\s([\d_]+)/),f=a.match(/(web[o0]s)/),g=a.match(/(windows)\D+([\d.]+)/),h=a.match(/(linux)\s+([\d.]+)/);a=a.match(/\((\w+);/);c?(b.name="Android",b.version=c[2]||""):d?(b.name="iOS",b.version=d[2].replace(/_/g,".")||""):e?(b.name="iOS",b.version=e[2].replace(/_/g,".")||""):f?(b.name="webOS",b.version=""):g?(b.name="Windows",b.version=g[2]||""):h?(b.name=
"Linux",b.version=h[2]||""):(b.name=a?a[1]:"Not detected",b.version="");return b}(),J="Windows"==x.name||"on"===$.urlPar("is_windows"),K="Linux"==x.name||"on"===$.urlPar("is_linux"),r="webOS"==x.name||"on"===$.urlPar("is_smarttv"),D="Android"==x.name||"on"===$.urlPar("is_mobile");$("button.deviceinfo",m).click(function(){$(".panel.information #content").html("<br/>Device information<p>User agent: "+navigator.userAgent+'</p><p><div>Video FPS: <span class="fps">0.0</span></div><div>CPU FPS: <span class="cfps">0.0</span></div></p><p>Screen size: '+
window.screen.width+"x"+window.screen.height+"<br/>Device pixel ratio: "+window.devicePixelRatio+"</p><p>URL: "+document.URL+"</p><p>Platform: "+(r?"SmartTV":D?"Mobile":J?"Windows":K?"Linux":"Unknown")+"</p><p>OS: "+x.name+" "+x.version+"</p>");k(".information")});var E=!1,L=document.exitFullscreen||document.webkitExitFullscreen||document.webkitCancelFullScreen||document.mozCancelFullScreen||document.msExitFullscreen||function(){},y=document.documentElement,M=y.requestFullscreen||y.webkitRequestFullscreen||
y.mozRequestFullscreen||y.msRequestFullscreen||function(){};!r&&"on"!==$.urlPar("always_fullscreen")&&(document.fullscreenEnabled||document.webkitFullscreenEnabled||document.webkitCancelFullScreen||document.mozFullscreenEnabled||document.msFullscreenEnabled)?$("button.fullscreen",m).click(function(){E?(L.call(document),E=!1):(M.call(y),E=!0)}):$("button.fullscreen",m).hide();var C=$("select.select-model",m);$("select.select-model",m).hide();for(var F={},q=0;q<JSSpeccy.Spectrum.MODELS.length;q++)m=
JSSpeccy.Spectrum.MODELS[q],F[m.id]=m,C.append($("<option></option>").text(m.name).attr({value:m.id}));C.change(function(){var a=$(this).val();h.setModel(F[a])});e();h.onChangeModel.bind(e);var N=$("input.autoload-tapes");$("input.autoload-tapes").parent().hide();var v=$("input.checkerboard-filter");c(h.settings.checkerboardFilter.get());h.settings.checkerboardFilter.onChange.bind(c);v.change(function(){h.settings.checkerboardFilter.set(v.is(":checked"))});"on"!==$.urlPar("hq2x")?v.parent().hide():
v.attr("checked",!0);$(".panel button.close",g).click(function(){f()});m=$(".panel.open-file",g);var G=m.find("input[type='file']");G.change(function(){h.loadLocalFile(this.files[0],{autoload:N.is(":checked"),debugPrint:a.debugPrint});G.val("");f()});r&&"on"!==$.urlPar("load_file")&&$("#load_file",g).hide();var A=m.find("input[type='url']");m.find("button.open-url").click(function(){""!==A.val()&&(f(),h.stop(),h.setLoadUrlOnStart($("#links").children(":selected").attr("href")),h.start())});var w=
$(".panel.pokes",g);w.find("button.apply_pokes").click(function(){var b=w.find("input[type='text']").val(),c=!0;if(""!==b){b=$jscomp.makeIterator(b.split(":"));for(var d=b.next();!d.done;d=b.next()){var e=$jscomp.makeIterator(d.value.split(",").map(Number));d=e.next().value;e=e.next().value;a.debugPrint&&console.log("Poke ",d," ",e);if(16384<=d&&65536>d&&0<=e&&256>e)h.poke(d,e);else{$("#error_text",w).text("Addresses or values are wrong");c=!1;break}}}else $("#error_text",w).text("Pokes string is empty"),
c=!1;c&&f()});m=new URL(document.URL);m=m.protocol+"//"+(0<m.host.length?m.host+"/":"")+m.pathname.substring(0,m.pathname.lastIndexOf("/"));r?A.val("file://usb:0/"):A.val(m);(function(a,b){var c=new XMLHttpRequest;c.addEventListener("load",function(a){b(c.response)});c.open("GET",a);c.send()})("zx_spectrum_pokes.txt",function(a){w.find("textarea").val(a)});$.setJoystick=function(a){var b=a.split(",");up=b[0];down=b[1];left=b[2];right=b[3];fire=b[4];action=b[5];n("select_key_up",up);n("select_key_down",
down);n("select_key_left",left);n("select_key_right",right);n("select_key_fire",fire);n("select_key_action",action);r&&h.setKeymap(a)};$.setJoystick(document.getElementById("selcontrol").value);$.prev_keyupdown_time=performance.now();$.padTouch=function(a,b){a=document.getElementById(a);var c=a.options[a.selectedIndex];setTimeout(function(){h.keyboard().registerKeyDown(c.value)},Math.max($.key_timeout-(performance.now()-$.prev_keyupdown_time),0));$.prev_keyupdown_time=performance.now()};$.padUntouch=
function(a,b){a=document.getElementById(a);var c=a.options[a.selectedIndex];setTimeout(function(){h.keyboard().registerKeyUp(c.value)},Math.max($.key_timeout-(performance.now()-$.prev_keyupdown_time),0));$.prev_keyupdown_time=performance.now()};D||"on"===$.urlPar("padblock")||$(".padblock",g).hide();var H=document.onkeypress,I=!1;$("#typer",g).click(function(){h.keyboard().active?(a.debugPrint&&console.log("focus in on-screen keyboard"),H=document.onkeypress,document.onkeypress=function(){return!0},
I=$(".padblock",g).is(":visible"),$(".padblock",g).hide(),h.keyboard().active=!1,$("#typer",g).focus()):$("#typer",g).blur()});var u=229;$("#typer",g).keydown(function(a){u=a.keyCode;229!==u&&p(u,a.key)});$("#typer",g).on("input",function(a){229===u?(u=a.target.value.toUpperCase().charCodeAt(a.target.value.length-1),p(u,a.target.value)):u=229});$("#typer",g).focusout(function(){a.debugPrint&&console.log("focus out on-screen keyboard");$("#typer",g).val("");document.onkeypress=H;I&&$(".padblock",g).show();
h.keyboard().active=!0});r||D||"on"===$.urlPar("typer")||$("#typer-div",g).hide();r&&(document.addEventListener("keyboardStateChange",function(){event.detail.visibility||$("#typer",g).blur()}),document.addEventListener("keydown",function(a){console.log(a.key+" "+a.keyCode);a.keyCode==h.keyboard().TV_Blue&&$("#typer",g).click();a.keyCode==h.keyboard().TV_Back&&$("#typer",g).blur()}));setInterval(function(){$(".fps").text(h.getFps().toFixed(1));$(".cfps").text(h.getCfps().toFixed(1))},1E3);history.pushState(null,
null,location.href);window.onpopstate=function(){history.go(1);$("#typer",g).blur()};a.debugPrint?console.log("URL keys: ",url_pars_list):$(document).bind("contextmenu",function(a){a.preventDefault()});r&&setTimeout(function(){h.start()},0);return{}};
