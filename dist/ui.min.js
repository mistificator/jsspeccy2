...
JSSpeccy.UI=function(a){function b(){f.isRunning?D.removeClass("start").addClass("stop"):D.removeClass("stop").addClass("start")}function c(a){I.toggleClass("enabled",a)}function g(){E.val(f.getModel().id)}function d(a){a?(z.attr("checked",!0),$(h).addClass("hard-edged-pixels")):(z.removeAttr("checked"),$(h).removeClass("hard-edged-pixels"))}function k(a){$(".panel",h).hide();$(".panel",h).filter(a).show();f.deactivateKeyboard()}function l(){$(".panel",h).hide();f.activateKeyboard()}function e(a,
...
"Linux",b.version=h[2]||""):(b.name=a?a[1]:"Not detected",b.version="");return b}(),S="Windows"==y.name||"on"===$.urlPar("is_windows"),T="Linux"==y.name||"on"===$.urlPar("is_linux"),t="webOS"==y.name||"on"===$.urlPar("is_smarttv"),F="Android"==y.name||"on"===$.urlPar("is_mobile");$("button.deviceinfo",p).click(function(){$(".panel.information #content").html("<br/>Device information<p>User agent: "+navigator.userAgent+'</p><p><div>Video FPS: <span class="fps">0.0</span></div><div>CPU FPS: <span class="cfps">0.0</span></div></p><p>Screen size: '+
window.screen.width+"x"+window.screen.height+"<br/>Device pixel ratio: "+window.devicePixelRatio+"</p><p>URL: "+document.URL+"</p><p>Platform: "+(t?"SmartTV":F?"Mobile":S?"Windows":T?"Linux":"Unknown")+"</p><p>OS: "+y.name+" "+y.version+"</p>");k(".information")});var G=!1,U=document.exitFullscreen||document.webkitExitFullscreen||document.webkitCancelFullScreen||document.mozCancelFullScreen||document.msExitFullscreen||function(){},A=document.documentElement,V=A.requestFullscreen||A.webkitRequestFullscreen||
A.mozRequestFullscreen||A.msRequestFullscreen||function(){};!t&&"on"!==$.urlPar("always_fullscreen")&&(document.fullscreenEnabled||document.webkitFullscreenEnabled||document.webkitCancelFullScreen||document.mozFullscreenEnabled||document.msFullscreenEnabled)?$("button.fullscreen",p).click(function(){G?(U.call(document),G=!1):(V.call(A),G=!0,x&&$("button.menu",p).trigger("click"))}):$("button.fullscreen",p).hide();var E=$("select.select-model",p);$("select.select-model",p).hide();for(var J={},r=0;r<
JSSpeccy.Spectrum.MODELS.length;r++){var n=JSSpeccy.Spectrum.MODELS[r];J[n.id]=n;E.append($("<option></option>").text(n.name).attr({value:n.id}))}E.change(function(){var a=$(this).val();f.setModel(J[a])});g();f.onChangeModel.bind(g);var W=$("input.autoload-tapes");$("input.autoload-tapes").parent().hide();var z=$("input.checkerboard-filter");d(f.settings.checkerboardFilter.get());f.settings.checkerboardFilter.onChange.bind(d);z.change(function(){f.settings.checkerboardFilter.set(z.is(":checked"))});
z.attr("checked","on"===$.urlPar("hq2x"));var H=$("input.audio-hifi");H.change(function(){f.setAudioHiFi(H.is(":checked"))});H.attr("checked","on"===$.urlPar("audio_hifi"));$(".panel button.close",h).click(function(){l()});n=$(".panel.open-file",h);var K=n.find("input[type='file']");K.change(function(){l();f.stop();f.loadLocalFile(this.files[0],{autoload:W.is(":checked"),debugPrint:a.debugPrint});K.val("")});t&&"on"!==$.urlPar("load_file")&&$("#load_file",h).hide();t&&"on"!==$.urlPar("save_file")&&
$("#save_file",h).hide();var C=n.find("input[type='url']");n.find("button.open-url").click(function(){var b=C.val();""!==b&&(l(),f.stop(),f.setLoadUrlOnStart(a.corsProxy+b),f.start(),$.urlPar("load",b))});n.find("button.save-file").click(function(){var a=$("#index option:selected").text();if(0==a.length||"-"==a)a="snapshot_"+(new Date(Date.now())).toISOString().replace(/[:\.\-TZ]+/g,"_");""!==a&&(f.stop(),f.saveZ80Snapshot(a.replace(/[\s]+/g,"_")+".z80"),l())});var u=$(".panel.pokes",h);u.find("button.apply_pokes").click(function(){var b=
u.find("input[type='text']").val(),c=!0;if(""!==b){b=$jscomp.makeIterator(b.split(":"));for(var d=b.next();!d.done;d=b.next()){var e=$jscomp.makeIterator(d.value.split(",").map(Number));d=e.next().value;e=e.next().value;a.debugPrint&&console.log("Poke ",d," ",e);if(16384<=d&&65536>d&&0<=e&&256>e)f.poke(d,e);else{$("#error_text",u).text("Addresses or values are wrong");c=!1;break}}}else $("#error_text",u).text("Pokes string is empty"),c=!1;c&&l()});n=new URL(document.URL);n=n.protocol+"//"+(0<n.host.length?
n.host+"/":"")+n.pathname.substring(0,n.pathname.lastIndexOf("/"));t?C.val("file://usb:0/"):C.val(n.replace(a.corsProxy,""));(function(a,b){var c=new XMLHttpRequest;c.addEventListener("load",function(a){b(c.response)});c.open("GET",a);c.send()})("zx_spectrum_pokes.txt",function(a){u.find("textarea").val(a)});$.setJoystick=function(a){var b=a.split(",");up=b[0];down=b[1];left=b[2];right=b[3];fire=b[4];action=b[5];m("select_key_up",up);m("select_key_down",down);m("select_key_left",left);m("select_key_right",
right);m("select_key_fire",fire);m("select_key_action",action);t&&f.setKeymap(a)};$.setJoystick(document.getElementById("selcontrol").value);$.prev_keyupdown_time=performance.now();$.padTouch=function(a,b){a=document.getElementById(a);var c=a.options[a.selectedIndex];setTimeout(function(){f.keyboard().registerKeyDown(c.value)},Math.max($.key_timeout-(performance.now()-$.prev_keyupdown_time),0));$.prev_keyupdown_time=performance.now()};$.padUntouch=function(a,b){a=document.getElementById(a);var c=
a.options[a.selectedIndex];setTimeout(function(){f.keyboard().registerKeyUp(c.value)},Math.max($.key_timeout-(performance.now()-$.prev_keyupdown_time),0));$.prev_keyupdown_time=performance.now()};F||"on"===$.urlPar("padblock")||$(".padblock",h).hide();var L=document.onkeypress,M=!1;$("#typer",h).click(function(){f.keyboard().active?(a.debugPrint&&console.log("focus in on-screen keyboard"),L=document.onkeypress,document.onkeypress=function(){return!0},M=$(".padblock",h).is(":visible"),$(".padblock",
h).hide(),f.keyboard().active=!1,$("#typer",h).focus()):$("#typer",h).blur()});var w=229;$("#typer",h).keydown(function(a){w=a.keyCode;229!==w&&q(w,a.key)});$("#typer",h).on("input",function(a){229===w?(w=a.target.value.toUpperCase().charCodeAt(a.target.value.length-1),q(w,a.target.value)):w=229});$("#typer",h).focusout(function(){a.debugPrint&&console.log("focus out on-screen keyboard");$("#typer",h).val("");document.onkeypress=L;M&&$(".padblock",h).show();f.keyboard().active=!0});t||F||"on"===$.urlPar("typer")||
$("#typer-div",h).hide();t&&(document.addEventListener("keyboardStateChange",function(){event.detail.visibility||$("#typer",h).blur()}),document.addEventListener("keydown",function(a){console.log(a.key+" "+a.keyCode);a.keyCode==f.keyboard().TV_Blue&&$("#typer",h).click();a.keyCode==f.keyboard().TV_Back&&$("#typer",h).blur()}));setInterval(function(){$(".fps").text(f.getFps().toFixed(1));$(".cfps").text(f.getCfps().toFixed(1))},1E3);history.pushState(null,null,location.href);window.onpopstate=function(){history.go(1);
$("#typer",h).blur()};a.debugPrint?console.log("URL keys: ",url_pars_list):$(document).bind("contextmenu",function(a){0<$(h).find($(a.target)).length&&$(a.target).is("canvas")&&(console.log($(h).find($(a.target))),a.preventDefault())});t&&setTimeout(function(){f.start()},0);n=["#catalogue","#letter","#index","#links"];var N;(N=new DummyCat).init.apply(N,$jscomp.arrayFromIterable(n));var O;(O=new ZxCzCat(a.cors_proxy)).init.apply(O,$jscomp.arrayFromIterable(n));var P;(P=new Rgb2019Cat(a.cors_proxy)).init.apply(P,
$jscomp.arrayFromIterable(n));var Q;(Q=new WoSCat(a.cors_proxy)).init.apply(Q,$jscomp.arrayFromIterable(n));var R;(R=new ArchCat(a.cors_proxy)).init.apply(R,$jscomp.arrayFromIterable(n));return{}};
