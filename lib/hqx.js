...
window.hqx = function(src_img, dst_img) {
	...
	const height = src_img.height;
	const size = _src.length;
	const threshold = size / hqxThreshold;
	var cache_miss = 0;
	...
	if ((cache_miss + _prev_cache_miss) / 2 < threshold) {
		hq2x(
				src_img.width, 
				src_img.height
			);
	}
	else { // skip HQ2X, simple upscale
		if (hqxOptions.debugPrint) {
			console.log("Too much cache misses (", cache_miss, "), so skip HQ2X, simple upscale");
		}
		var i = 0;
		const width_2 = width * 2;
		const height_2 = height * 2;
		for (var y_2 = 0; y_2 < height_2; y_2 += 2) {
			const offs_2 = y_2 * width_2;
			for (var x_2 = 0; x_2 < width_2; i++, x_2 += 2) {
				if (_cache_dirty[i] > 0) {
					_dest[offs_2 + x_2] = _dest[offs_2 + x_2 + 1] = _dest[offs_2 + width_2 + x_2] = _dest[offs_2 + width_2 + x_2 + 1] = _src[i];
				}
			}
		}
	}
	_prev_cache_miss = cache_miss;
